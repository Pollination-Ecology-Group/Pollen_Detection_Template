...
while true; do
    POD_STATUS=$(./kubectl --kubeconfig=kubeconfig.yaml get pods -l job-name=wipe-s3-fast -n {{NAMESPACE}} -o jsonpath='{.items[0].status.phase}')
...
./kubectl --kubeconfig=kubeconfig.yaml delete job pollen-build-species-dataset -n {{NAMESPACE}} --ignore-not-found
./kubectl --kubeconfig=kubeconfig.yaml apply -f k8s/pollen-build-species-dataset-job.yaml

echo "Waiting for Dataset Builder..."
while true; do
    POD_STATUS=$(./kubectl --kubeconfig=kubeconfig.yaml get pods -l job-name=pollen-build-species-dataset -n {{NAMESPACE}} -o jsonpath='{.items[0].status.phase}' 2>/dev/null)
    if [ "$POD_STATUS" = "Succeeded" ]; then 
...
aws --endpoint-url https://s3.cl4.du.cesnet.cz s3 sync s3://bucket/{{S3_PREFIX}}/Species_model/ ~/Desktop/Species_model --no-verify-ssl
