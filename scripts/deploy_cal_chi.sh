...
$KUBECTL delete job pollen-{{JOB_ID}}-infer -n $NAMESPACE --ignore-not-found

# 5. Deploy
echo "🚀 5. Deploying Cal_chi inference job..."
sed "s|IMAGE_PLACEHOLDER|${IMAGE_NAME}|g" k8s/pollen-{{JOB_ID}}-infer-job.yaml \
...
count=0
while ! $KUBECTL logs job/pollen-{{JOB_ID}}-infer -n $NAMESPACE > /dev/null 2>&1; do
    echo -n "."
    sleep 10
    count=$((count+1))
    [ $count -ge 120 ] && { echo ""; echo "❌ Timeout waiting for logs. Check: $KUBECTL describe job pollen-{{JOB_ID}}-infer -n $NAMESPACE"; exit 1; }
done
echo ""
echo "👀 7. Streaming Python logs (Ctrl+C to detach)..."
$KUBECTL logs -f job/pollen-{{JOB_ID}}-infer -n $NAMESPACE
