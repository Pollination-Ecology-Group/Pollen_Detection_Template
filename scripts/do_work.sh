...
$KUBECTL --kubeconfig=$KUBECONFIG delete job wipe-s3-fast -n {{NAMESPACE}} --ignore-not-found
$KUBECTL --kubeconfig=$KUBECONFIG apply -f k8s/wipe-s3-job.yaml
$KUBECTL --kubeconfig=$KUBECONFIG wait --for=condition=complete job/wipe-s3-fast -n {{NAMESPACE}} --timeout=300s || true

echo "2/4: Triggering new Species Generation Job with the updated general model..."
$KUBECTL --kubeconfig=$KUBECONFIG delete job pollen-build-species-dataset -n {{NAMESPACE}} --ignore-not-found
...
$KUBECTL --kubeconfig=$KUBECONFIG wait --for=condition=complete job/pollen-build-species-dataset -n {{NAMESPACE}} --timeout=3600s
